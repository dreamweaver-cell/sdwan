...
def clitemplate(self, device_variables, device_template):
...
def response(self, method, headers=None, payload=None, files=None, timeout=10):
    with open(f"{mypath}/support/http_respone.obj", "rb") as f:
...
def device_tmpl_list(self, factory_default=False, name_list=None):
...
def return_dict(self, template_id, device_id_list=None):
...
def mock_get_template_input(monkeypatch):
    """Patches template_data.export_device_template_list"""
    monkeypatch.setattr(SnapDeviceTemplates, "get_template_input", return_dict)
