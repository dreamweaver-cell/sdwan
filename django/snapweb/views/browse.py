...
def _get_abs_virtual_root():
    return _eventual_path(settings.SNAPCONFIG_DIRECTORY)
...
def _filter_files(files):
    contents = []
    for item in files:
        if re.match(settings.HOSTNAME_REGEXP, item):
...
def get_names(directory):
    ...
    contents = _filter_files(os.listdir(directory))
    files, directories = [], []
...
def _list_directory(request, directory):
    ...
    if check_access(request):
        ...
        template = getattr(settings, 'DIRECTORY_TEMPLATE', 'list.html')
...
def browse(request, path=""):
    """ Directory list view """
    eventual_path = _eventual_path(os.path.join(settings.SNAPCONFIG_DIRECTORY, path))
...
def browse_updated(request, path):
    """ Directory list view """
    eventual_path = _eventual_path(os.path.join(settings.MEDIA_URL, path))
